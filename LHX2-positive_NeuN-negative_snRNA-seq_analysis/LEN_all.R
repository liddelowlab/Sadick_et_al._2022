#LIM Homeobox 2 (LHX2)-positive/NeuN-negative snRNA-seq analysis for *ALL* donors characterized
#Goal: To evaluate astrocyte and oligodendrocyte heterogeneity in APOE e2/3 Alzheimer's disease and age-matched non-symptomatic patients.
...
#Load raw counts
fastq_dirs <- list.dirs("file_path/GRCh38_premRNA_CR4_LEN", recursive = FALSE, full.names = TRUE)
...
#Load metadata 
md_dir <- file.path("file_path/GRCh38_premRNA_CR4_LEN", "metadata_LEN_all.xlsx")
...
#SAVE SCE
saveRDS(sce, file.path("file_path", "CR4_LEN_all_SCE.rds"))
...
#Load SCE
sce <- readRDS(file.path("file_path", "CR4_LEN_all_SCE.rds"))
...
#SAVE SeuratObject
saveRDS(so, file.path("file_path", "CR4_LEN_all_so_30PC.rds"))
...
write.csv(table(sce$cluster_id, sce$sample_id), "file_path/LEN_all_so_PC30_res0.1_cluster_numbers.csv")
...
write.csv(feature_by_sample_table, "file_path/LEN_all_so_PC30_res0.1_QC_feature_by_sample.csv")
...
write.csv(count_by_sample_table, "file_path/LEN_all_so_PC30_res0.1_QC_count_by_sample.csv")
...
write.csv(feature_by_cluster_table, "file_path/LEN_all_so_PC30_res0.1_QC_feature_by_cluster.csv")
...
write.csv(count_by_cluster_table, "file_path/LEN_all_so_PC30_res0.1_QC_count_by_cluster.csv")
...
write.csv(so.markers, "file_path/LEN_all_so_PC30_res0.1_genes-RNA.csv")
...
so.renamed <- RenameIdents(so, `0` = "Astro_A", `1` = "Oligo", `2` = "Astro_B", `3` = "Neuro_A", `4` = "Endo", `5`= "Micro", `6` = "OPC", `7`= "Neuro_B", `8`= "Neuro_C", `9`= "Astro_C")
DefaultAssay(so.renamed) <- "RNA"
DimPlot(so.renamed, reduction = "tsne") + theme(aspect.ratio = 1)

#Reorder levels of ident
so_copy <- so.renamed
designated_levels <- c("Astro_A", "Astro_B", "Astro_C", "OPC", "Oligo", "Neuro_A", "Neuro_B", "Neuro_C", "Micro", "Endo")
...
write.csv(so_copy_cluster.averages[["RNA"]], "file_path/LEN_all_so_PC30_res0.1_avg_exp_by_cluster.csv")
...
saveRDS(so_astro, file.path("file_path", "CR4_LEN_all_so_astro_unint.rds"))
saveRDS(so_oligo, file.path("file_path", "CR4_LEN_all_so_oligo_unint.rds"))
...
#Load data
so_astro <- readRDS(file.path("file_path", "CR4_LEN_all_so_astro_unint.rds"))
...
as <- FindIntegrationAnchors(so_astro, verbose = TRUE, k.filter = 110)
...
#Save SeuratObject
saveRDS(so_astro, file.path("file_path", "CR4_LEN_all_so_astro_20PC.rds"))
...
#Load data and convert to SCE
so_astro <- readRDS(file.path("file_path", "CR4_LEN_all_so_astro_20PC.rds"))
sce <- as.SingleCellExperiment(so_astro, assay = "RNA")
colData(sce) <- as.data.frame(colData(sce)) %>% 
  mutate_if(is.character, as.factor) %>% 
  DataFrame(row.names = colnames(sce))
...
(n_cells <- table(sce$cluster_id, sce$sample_id))
write.csv(table(sce$cluster_id, sce$sample_id), "file_path/LEN_all_so_astro_20PC_res0.4_numbers.csv")
...
write.csv(so_astro.markers, "file_path/LEN_all_so_astro_20PC_res0.4_genes-RNA.csv")
...
##Visualization of cluster 7 unique features
so_astro_C7 <- subset(x = so_astro, idents = c(7), invert = FALSE)
Idents(so_astro_C7) <- "sample_id"
designated_levels <- c("D1_DMEM", "D1_FACS", "D2", "D3", "D4", "D5", "D6", "D7", "D8", "D9", "D10", "D11", "D12", "D13", "D15", "D16", "D17")
Idents(so_astro_C7) <- factor(Idents(so_astro_C7), levels= designated_levels)
DefaultAssay(so_astro_C7) <- "RNA"
DimPlot(so_astro_C7, reduction = "tsne", pt.size = 0.001) + theme(aspect.ratio = 1) +scale_color_manual(values = usecol("pal_unikn_pair", 17))

DotPlot(so_astro_C7, features = c("WARS","PARP14","HSPA6","RSAD2","DDX58","HSPD1","HSP90AA1","IFIT3","HSPH1","HSPA1A","TXNRD1","SOD2","MX1","BAG3","IFIT2","SAT1","IFI6","IFI44L","DNAJA4","DNAJB1","IFIT1","TRIM22","GBP1","ISG15","CRYAB","HSP90AB1","HSPE1","UBB","HELZ2","EPSTI1","ZC3HAV1","DNAJB6","HSPA8","EIF2AK2","IFI44","AC083837.1","TRIM25","NAMPT","MAP2","SERPINH1","STAT2","AC096637.1","HERC6","PLSCR1","MAP1B","SP110","IFIH1","GCLM","DNAJA1","LAP3"), cols = c("lightgrey", "#611ac6")) + theme(axis.text.x = element_text(angle = 90, hjust=1))
...
DimPlot(so_astro, reduction = "tsne", group.by = "disease_id", pt.size = 0.001) + theme(aspect.ratio = 1)
...
write.csv(so_astro_cluster.averages[["RNA"]], "file_path/LEN_all_so_astro_20PC_res0.4_avg_exp_by_cluster.csv")
...
DoHeatmap(so_astro_cluster.averages, features = c("HPSE2","CABLES1","ZNF98","TRPM3","ARHGAP24","APOE","CST3","FTH1","ITM2C","HEPN1","DPP10","AC012405.1","ADAMTSL3","L3MBTL4","LINC00609","CHI3L1","SOD2","NAMPT","TXNRD1","WARS","DCLK1","SLC38A1","DPP6","LINC01411","ID3","DDIT4","JUNB","LINC01088","KAZN","PARP14","HSPA6","RSAD2","DDX58","PTCHD4","KCNMB2-AS1","MIR34AHG","AL353138.1","ZMAT3","MOXD1","DACH1","SGCD","TMEM132C","LTBP1","XIST","RIMS1","CHST11","SYNE1","GNG7"), size = 3, draw.lines = FALSE, group.colors = usecol("pal_unikn_pair")) +scale_fill_gradientn(colours = (brewer.pal(9,name = "Purples")))
...
#Load data
so_oligo <- readRDS(file.path("file_path", "CR4_LEN_all_so_oligo_unint.rds"))
...
as <- FindIntegrationAnchors(so_oligo, verbose = FALSE, k.filter = 70)
so_oligo <- IntegrateData(anchorset = as, dims = seq_len(30), verbose = FALSE)
...
#Save SeuratObject
saveRDS(so_oligo, file.path("file_path", "CR4_LEN_all_so_oligo_15PC.rds"))
...
#Load data and convert to SCE
so_oligo <- readRDS(file.path("file_path", "CR4_LEN_all_so_oligo_15PC.rds"))
sce <- as.SingleCellExperiment(so_oligo, assay = "RNA")
colData(sce) <- as.data.frame(colData(sce)) %>% 
  mutate_if(is.character, as.factor) %>% 
  DataFrame(row.names = colnames(sce))
...
so_oligo <- SetIdent(so_oligo, value = "integrated_snn_res.0.2")
so_oligo@meta.data$cluster_id <- Idents(so_oligo)
sce$cluster_id <- Idents(so_oligo)
(n_cells <- table(sce$cluster_id, sce$sample_id))
write.csv(table(sce$cluster_id, sce$sample_id), "file_path/LEN_all_so_oligo_15PC_res0.2_numbers.csv")
...
DimPlot(so_oligo, reduction = "tsne", pt.size = 0.001) + theme(aspect.ratio = 1)

#Find all markers
DefaultAssay(so_oligo) <- "RNA"
so_oligo.markers <- FindAllMarkers(so_oligo, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
write.csv(so_oligo.markers, "file_path/LEN_all_so_oligo_10PC_res0.2_genes-RNA.csv")

#Visualize
VlnPlot(so_oligo, features = c("ALDOC"), pt.size = 0)
...
write.csv(so_oligo_cluster.averages[["RNA"]], "file_path/LEN_all_so_oligo_10PC_res0.2_avg_exp_by_cluster.csv")
