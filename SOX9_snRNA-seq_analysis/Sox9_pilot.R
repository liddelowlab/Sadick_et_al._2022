#SOX9 snRNA-seq analysis
#Goal: To evaluate if SOX9 sorting enrichs for astrocytic nuclei by evaluating both SOX9-positive and SOX9-negative nuclei by snRNA-seq
...
#Load raw counts
fastq_dirs <- list.dirs("file_path/GRCh38_premRNA_CR4_Sox9", recursive = FALSE, full.names = TRUE)
...
#Load metadata
md_dir <- file.path("file_path/GRCh38_premRNA_CR4_Sox9", "metadata_sox9_pilot.xlsx")
...
#SAVE SCE
saveRDS(sce, file.path("file_path", "CR4_Sox9_SCE.rds"))
...
#Load SCE
sce <- readRDS(file.path("file_path", "CR4_Sox9_SCE.rds"))
...
#SAVE SeuratObject
saveRDS(so, file.path("file_path", "CR4_Sox9_so_30PC.rds"))
...
so <- readRDS(file.path("file_path", "CR4_Sox9_so_30PC.rds"))
...
write.csv(table(sce$cluster_id, sce$sample_id), "file_path/Sox9_so_PC30_res0.1_cluster_numbers.csv")
...
write.csv(feature_by_sample_table, "file_path/Sox9_so_PC30_res0.1_cluster_QC_feature_by_sample.csv")
...
write.csv(count_by_sample_table, "file_path/Sox9_so_PC30_res0.1_cluster_QC_count_by_sample.csv")
...
write.csv(feature_by_cluster_table, "file_path/Sox9_so_PC30_res0.1_cluster_QC_feature_by_cluster.csv")
...
write.csv(count_by_cluster_table, "file_path/Sox9_so_PC30_res0.1_cluster_QC_count_by_cluster.csv")

#DETERMINE WHAT CELL TYPES ARE PRESENT IN DATASET BEFORE MAKING NEW SEURAT OBJECTS
DefaultAssay(so) <- "RNA"
DimPlot(so, reduction = "tsne", pt.size = 0.001) + theme(aspect.ratio = 1) +scale_color_manual(values = usecol("pal_unikn_pair", 22))

#Find all markers
DefaultAssay(so) <- "RNA"
so.markers <- FindAllMarkers(so, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
write.csv(so.markers, "file_path/Sox9_so_PC30_res0.1_cluster_genes-RNA.csv")
...
so.renamed <- RenameIdents(so, `0` = "Neuro_A", `1` = "Oligo", `2` = "Astro", `3` = "Neuro_B", 
                           `4` = "Micro", `5`= "Neuro_C", `6` = "Neuro_D", `7`= "Neuro_E", `8`= "Neuro_F", `9` = "Neuro_G", `10` = "OPC", `11` = "Neuro_H", `12` = "Neuro_I", `13` = "Neuro_J", `14`= "Neuro_K", `15` = "Endo", `16`= "Neuro_L", `17`= "Neuro_M", `18` = "Neuro_N", `19`= "Neuro_O", `20`= "Endo-like_A", `21`= "Endo-like_B")
DefaultAssay(so.renamed) <- "RNA"
DimPlot(so.renamed, reduction = "tsne") + theme(aspect.ratio = 1)

#Reorder levels of ident
so_copy <- so.renamed
designated_levels <- c("Astro", "OPC", "Oligo", "Neuro_A", "Neuro_B", "Neuro_C", "Neuro_D", "Neuro_E", "Neuro_F", "Neuro_G", "Neuro_H", "Neuro_I", "Neuro_J", "Neuro_K", "Neuro_L", "Neuro_M", "Neuro_N", "Neuro_O", "Micro", "Endo", "Endo-like_A", "Endo-like_B")
...
DimPlot(so_copy, reduction = "tsne", group.by = "enrichment_id", pt.size = 0.001, cols = c("#5698af", "#e6cc4f")) + theme(aspect.ratio = 1)
...
write.csv(so_copy_cluster.averages[["RNA"]], "file_path/Sox9_so_avg_exp_by_cluster.csv")
...
DoHeatmap(so_copy_cluster.averages, features = c("AQP4", "GJA1", "ALDH1L1", "SLC1A3", "PDGFRA", "CSPG4", "SOX10", "PLP1", "MBP", "MOG", "SNAP25", "STMN2", "GLS", "GAD1", "C1QB", "TYROBP", "P2RY12", "CLDN5", "NOSTRIN", "PECAM1"), size = 3,
...
##Removed donor 5A because had 4 astros, next lowest is >100
so_astro_1 <- subset(x = so.renamed, subset = sample_id == "5A", invert = TRUE)
so_astro <- subset(x = so_astro_1, idents = c("Astro"), invert = FALSE)
...
as <- FindIntegrationAnchors(so_astro, verbose = TRUE, k.filter = 105)
...
#Save SeuratObject
saveRDS(so_astro, file.path("file_path/", "CR4_Sox9_so_astro_10PC.rds"))
...
#Load data and convert to SCE
so_astro <- readRDS(file.path("file_path", "CR4_Sox9_so_astro_10PC.rds"))
sce <- as.SingleCellExperiment(so_astro, assay = "RNA")
colData(sce) <- as.data.frame(colData(sce)) %>% 
  mutate_if(is.character, as.factor) %>% 
  DataFrame(row.names = colnames(sce))
...
(n_cells <- table(sce$cluster_id, sce$sample_id))
write.csv(table(sce$cluster_id, sce$sample_id), "file_path/Sox9_so_astro_10PC_res0.4_numbers.csv")
...
write.csv(so_astro.markers, "file_path/Sox9_so_astro_10PC_res0.4_genes-RNA.csv")
...
write.csv(so_astro_cluster.averages[["RNA"]], "file_path/Sox9_so_astro_10PC_res0.4_avg_exp_by_cluster.csv")
...
DoHeatmap(so_astro_cluster.averages, features = c("AQP4", "GFAP", "FGFR3", "CLDN10", "GJA1", "ALDH1L1", "SLC1A3", "SLC1A2", "PDGFRA", "CSPG4", "SOX10", "PLP1", "MBP", "MOG", "OLIG2", "C1QB", "TYROBP", "SNAP25", "STMN2", "SLC17A7", "GAD1", "CLDN5", "NOSTRIN", "PDGFRB", "ANPEP", "FLT1", "ACTA2", "PECAM1", "VWF"), size = 3, 
          draw.lines = FALSE)
